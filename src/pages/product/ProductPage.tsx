        ...
        <Form layout="vertical" form={form} onFinish={handleSave}>
          ...
          <Form.Item label="Hình ảnh">
            ...
            <div style={{ color: "#888", fontSize: 12, marginTop: 6 }}>
              Ghi chú: Khi chỉnh sửa, nếu bạn không upload ảnh mới thì ảnh cũ sẽ được giữ. Nếu upload ít nhất 1 ảnh mới, backend (theo logic bạn cung cấp) sẽ xóa ảnh cũ và lưu ảnh mới.
            </div>
          </Form.Item>
      ...
      >
        {viewingProduct ? (
          <Descriptions bordered column={1}>
            ...
            /></Descriptions.Item>
            <Descriptions.Item label="Danh mục">{(viewingProduct.category && typeof viewingProduct.category === "object") ? viewingProduct.category.name : viewingProduct.category ?? "-"}</Descriptions.Item>
...
// try {
//   // Lọc files mới (có originFileObj) — chỉ gửi file mới lên server
//   const newFiles = uploadFileList.filter(f => !!(f as any).originFileObj);
//   let formData: FormData | null = null;

//   if (!editingProduct) {
//     // CREATE: cần formData và ít nhất 1 ảnh
//     if (newFiles.length === 0) {
//       message.error("Vui lòng chọn ít nhất 1 ảnh để tạo sản phẩm");
//       return;
//     }
...
//     formData.append("category", values.category ?? "");
//     newFiles.forEach(f => {
//       if ((f as any).originFileObj) formData!.append("images", (f as any).originFileObj as File);
//     });
//   } else {
//     // EDIT: nếu có file mới -> gửi FormData; nếu không -> gửi null để backend giữ ảnh cũ
//     if (newFiles.length > 0) {
//       formData = new FormData();
//       formData.append("name", values.name);
//       formData.append("description", values.description ?? "");
...
//       });
//     } else {
//       // khi không có file mới, vẫn muốn update text fields => backend PUT non-multipart
//       formData = null;
//     }
//   }

//   if (editingProduct) {
//     // updateProduct chấp nhận FormData | null
//     await updateProduct(editingProduct._id!, formData);
//     message.success("Cập nhật thành công");
//   } else {
//     await createProduct(formData!); // đã chắc chắn formData != null ở nhánh tạo
