...
import {
  getListBSCT, createListBSCT, updateListBSCT, deleteListBSCT, getListBSCTById, getCategoriesBSCT,
} from "../../api/bsctApi";
...
export interface CategoriesBSCT {
  _id: string;
  name: string;
}
...
export interface BSCT {
  ...
  isMoi?: boolean;
  images?: string;
  categoryBSCT?: CategoriesBSCT | string;
  createdAt?: string;
...
const BSCTAdmin: React.FC = () => {
  const [bscts, setBscts] = useState<BSCT[]>([]);
  const [categorieBSCTs, setCategorieBSCTs] = useState<CategoriesBSCT[]>([]);
  ...
  const [editingBlog, setEditingBlog] = useState<BSCT | null>(null);
  const [viewingBlog, setViewingBlog] = useState<BSCT | null>(null);
  ...
  const fetchData = async (page = 1, pageSize = DEFAULT_PAGE_SIZE, searchTerm = "", categoryBSCTId: string | null = null) => {
    try {
      setLoading(true);
      const data = await getListBSCT(page, pageSize, searchTerm, categoryBSCTId);
      setBscts(data.bscts ?? []);
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const cats = await getCategoriesBSCT();
        setCategorieBSCTs(cats ?? []);
      } catch (err) {
        console.error(err);
      }
    })();
  ...
  const openEditModal = (b: BSCT) => {
    ...
    form.setFieldsValue({
      ...
      isActive: b.isActive,
      isMoi: b.isMoi,
      categoryBSCT: typeof b.categoryBSCT === "string" ? b.categoryBSCT : (b.categoryBSCT?._id ?? ""),
    });
  ...
  const openViewModal = async (id?: string) => {
    if (!id) return;
    try {
      const res = await getListBSCTById(id);
      setViewingBlog(res);
      setViewVisible(true);
    } catch (err) {
      console.error(err);
  ...
  const handleSave = async (values: any) => {
    try {
      ...
      if (!editingBlog) {
        ...
        fd.append("categoryBSCT", values.categoryBSCT);

        const file = (uploadFileList[0] as any).originFileObj as File;
        fd.append("images", file);

        await createListBSCT(fd);
    ...
    () => [
      {
        ...
        dataIndex: "images",
        render: (url: string) =>
          url ? <img src={url} style={{ width: 100, height: 100, objectFit: "cover", borderRadius: 6 }} /> : "-",
      },
      { title: "Tiêu đề", dataIndex: "title" },
      {
        title: "Danh mục",
        dataIndex: "categoryBSCT",
        render: (cat: BSCT["categoryBSCT"]) => (typeof cat === "object" ? (cat as any)?.name : cat),
      },
      {
        title: "Ngày tạo",
        dataIndex: "createdAt",
    ...
    <div style={{ padding: 20 }}>
      ...
      >
        <Form form={form} layout="vertical" onFinish={handleSave}>
          ...
          <Form.Item name="categoryBSCT" label="Danh mục" rules={[{ required: true, message: "Chọn danh mục" }]}>
            <Select
              placeholder="Chọn danh mục"
              options={categorieBSCTs.map((c) => ({ label: c.name, value: c._id }))}
            />
          </Form.Item>
        ...
        {viewingBlog ? (
          <Descriptions bordered column={1}>
            ...
            <Descriptions.Item label="Danh mục">
              {typeof viewingBlog.categoryBSCT === "object" ? (viewingBlog.categoryBSCT as any)?.name : viewingBlog.categoryBSCT}
            </Descriptions.Item>
