...
import {getCategoriesThuVien, createCategoryThuVien, updateCategoryThuVien, deleteCategoryThuVien, } from "../../api/thuvien/catThuVienApi"

const CategoryThuVienPage: React.FC = () => {
  ...
  const fetchData = async () => {
    try {
      setLoading(true);
      const res = await getCategoriesThuVien();
      setData(res.data);
    } catch {
  ...
  const handleSave = async (values: any) => {
    try {
      if (editingCategory) {
        await updateCategoryThuVien(editingCategory._id, values);
        showAlert("success", "Cập nhật danh mục thành công");
      } else {
        await createCategoryThuVien(values);
  ...
  const handleDelete = async (id: string) => {
    try {
      await deleteCategoryThuVien(id);
...
export default CategoryThuVienPage;
