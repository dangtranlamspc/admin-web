...
import {getCategoryNNDT, updateCategoryNNDT, deleteCategoryNNDT, createCategoryNNDT} from "../../api/nndt/categoryNNDTApi"

const CategoryNNDTPage: React.FC = () => {
  ...
  const fetchData = async () => {
    try {
      setLoading(true);
      const res = await getCategoryNNDT();
      setData(res.data);
    } catch {
  ...
  const handleSave = async (values: any) => {
    try {
      if (editingCategory) {
        await updateCategoryNNDT(editingCategory._id, values);
        showAlert("success", "Cập nhật danh mục thành công");
      } else {
        await createCategoryNNDT(values);
...
export default CategoryNNDTPage;
